...
namespace TreinoMais.AcessoDados.Repositorios
{
    public class AlunoRepositorio : RepositorioGenerico<Aluno>, IAlunoRepositorio
    {
        ...
        public new async Task<IEnumerable<Aluno>> PegarTodos(int AdministradorId)
        {

            return await _contexto.Alunos.Include(a => a.Objetivo).Include(a => a.Professor).Where(a => a.Professor.AdministradorId == AdministradorId).ToListAsync();
        }
    }
}
